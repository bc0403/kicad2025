...
#define REFCLK 180000000UL  // 180 MHz
// safe default frequency for dummy mode (1 MHz)
#define SAFE_FREQ 1000000

static inline uint32_t calcFTW(uint32_t frequency) {
  // FTW = frequency * 2^32 / REFCLK
  // 用 64-bit 整数避免浮点误差
  return (uint32_t)(((uint64_t)frequency << 32) / (uint64_t)REFCLK);
}
...
void SetFreq(uint32_t frequency) {
  uint32_t ftw = calcFTW(frequency);

  // 发送 32-bit tuning word（LSB first，保持你原来的位序）
  for (int i = 0; i < 32; i++) {
    digitalWrite(DATA, (ftw >> i) & 0x1);
    // delay(1);
    // delayMicroseconds(1);
  ...
  // control byte: bit0=1 开启 6x（按你原逻辑）
  uint8_t ctrl = 0x01;
  for (int i = 0; i < 8; i++) {
    digitalWrite(DATA, (ctrl >> i) & 0x1);
    // delay(1);
    // delayMicroseconds(1);
    digitalWrite(WCLK, HIGH);
    // delay(1);
...
bool performSweep(bool checkForCommand) {
  ...
  for (count = freq_start; count <= freq_stop; count = count + freq_step) {
    ...
    uint16_t mag_mV   = (uint16_t)(value  * 3300UL / ((1UL << RESOLUTION) - 1));
    uint16_t phase_mV = (uint16_t)(value2 * 3300UL / ((1UL << RESOLUTION) - 1));
